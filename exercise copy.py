...
def generate_frames():
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            cv2.putText(image, f"Exercise: {['', 'Squat', 'Curl', 'Sit-up', 'Lunge', 'Pushup'][exercise_choice]}",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(image, f"Reps: {exercise_counters[exercise_choice]}",
                        (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(image, f"Time Left: {int(time_left)}s",
